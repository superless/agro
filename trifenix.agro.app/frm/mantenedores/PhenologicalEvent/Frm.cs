...
namespace trifenix.agro.app.frm.mantenedores.phenologicalEvent
{
    public partial class Frm : Form, IFenixForm
    {
        ...
        MapperConfiguration map = new MapperConfiguration(cfg => {
            cfg.CreateMap<PhenologicalEvent, PhenologicalEventInput>();
        });
        ...
        public string GetEntityName() => Cloud.GetCosmosEntityName<PhenologicalEvent>();

        public string FriendlyName() => "Evento Fenológico";
        ...
        {
            bsMain.EndEdit();
            var current = (PhenologicalEvent)obj;
            var m = map.CreateMapper();
            Cloud.PushElement(m.Map<PhenologicalEventInput>(current), entityName).Wait();
        ...
        {
            bsMain.EndEdit();
            var current = (PhenologicalEvent)bsMain.Current;
            var m = map.CreateMapper();
            Cloud.PushElement(m.Map<PhenologicalEventInput>(current), entityName).Wait();
        ...
        public object GetList() {
            ...
            var lst = Cloud.GetElements<PhenologicalEvent>(EntityRelated.PHENOLOGICAL_EVENT).ToList();
            var bl = new BindingList<PhenologicalEvent>(lst);
            bl.AllowNew = true;
            bl.AllowEdit = true;
            bl.AllowRemove = true;
            return bl;
        ...
        public string Description() => new MdmDocs().GetInfoFromEntity((int)EntityRelated.JOB).Description;
