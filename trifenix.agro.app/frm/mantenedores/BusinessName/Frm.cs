...
namespace trifenix.agro.app.frm.mantenedores.businessname
{
    public partial class Frm : Form, IFenixForm
    {
        ...
        public bool Valida()
        {
            var name = !string.IsNullOrWhiteSpace(tbxName.Text);
            if (!name)
            {
                ValidationForm.SetError(tbxName, "Nombre es obligatorio");
                return false;
            }

            var mail = !string.IsNullOrWhiteSpace(tbxEmail.Text);
            if (!mail)
            {
                ValidationForm.SetError(tbxEmail, "El; correo es obligatorio");
                return false;
            }
            var rut = !string.IsNullOrWhiteSpace(tbxRut.Text);
            if (!rut)
            {
                ValidationForm.SetError(tbxRut, "Rut es obligatorio");
                return false;
            }
            var web = !string.IsNullOrWhiteSpace(tbxWebPage.Text);
            if (!web)
            {
                ValidationForm.SetError(tbxWebPage, "Página web es obligatorio");
                return false;
            }
            var phone = !string.IsNullOrWhiteSpace(tbxPhone.Text);
            if (!phone)
            {
                ValidationForm.SetError(tbxPhone, "Telefono es obligatorio");
                return false;
            }
            return true;

        }
        public string GetEntityName() => Cloud.GetCosmosEntityName<BusinessName>();

        public string FriendlyName() => "Razón Social";
        ...
        public void Edit(object obj)
        {
            ...
            Cloud.PushElement(new BusinessNameInput { 
                Id = current.Id,
                Email = tbxEmail.Text,
                Rut = tbxRut.Text,
                Giro = tbxGiro.Text,
        ...
        public void ChangedList(object obj) {
            ...
            tbxGiro.Text = current.Giro;
            tbxPhone.Text = current.Phone;
            
            gbxItem.Text = $"Razón Social {tbxName.Text}";
        }

        public object GetList() => Cloud.GetElements<BusinessName>(EntityRelated.BUSINESSNAME);
        public string Description() => new MdmDocs().GetInfoFromEntity((int)EntityRelated.BUSINESSNAME).Description;
