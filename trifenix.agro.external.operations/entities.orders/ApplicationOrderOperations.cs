...
namespace trifenix.agro.external.operations.entities.orders
{
    public class ApplicationOrderOperations : IApplicationOrderOperations {
        ...
        private async Task<ApplicationOrder> GetApplicationOrder(string id, ApplicationOrderInput input) {var varietyIds = input.Applications.Any(s => s.Doses != null)? input.Applications.Where(s => s.Doses != null).SelectMany(s => s.Doses.IdVarieties).Distinct() : new List<string>();
        ...
        public async Task<ExtGetContainer<OrderResult>> GetApplicationOrdersByPage(string search, int page, int quantity, bool orderByDesc) {
            var searchLocal = new AgroSearch("agrisearch", "F9189208F49AF7C3DFD34E45A89F19E4");
            var ordersearch = searchLocal.GetOrders(search, page, quantity, orderByDesc);
            if (string.IsNullOrWhiteSpace(search))
                return await GetApplicationOrdersByPage(page, quantity, orderByDesc);
